...
#         cdist_array = cdist(ref[list(subgraphs[0])], ref[list(subgraphs[1])])
#         min_d = np.min(cdist_array)
#         s1, s2 = np.where(cdist_array == min_d)
...
if __name__ == '__main__':
    ...
    for i, c in enumerate(mol2.coords):
        rmsd, maxdev = rmsd_and_max(mol1.coords[0], c, center=True)
        print()
    ...
    #     graph = graphize(mol1.atoms[0], mol1.atomnos)
